...
class GeneralLedgerApiClientTest {
  ...
  inner class FindSubAccount {
    ...
    fun `should return first sub-account when found`() {
      ...
      val subRef = "SPND"
      val expectedResponse = GlSubAccountResponse(UUID.randomUUID(), UUID.randomUUID(), subRef, Instant.now(), "user")
  ...
  inner class CreateSubAccount {
    ...
    fun `should create sub-account successfully`() {
      ...
      val subRef = "SPND"
      val expectedResponse = GlSubAccountResponse(UUID.randomUUID(), parentId, subRef, Instant.now(), "user")
  ...
  inner class FindAccountByReference {
    ...
    fun `should return account when found`() {
      val ref = "A1234AA"
      val expectedResponse = GlAccountResponse(UUID.randomUUID(), ref, Instant.now(), "user", emptyList())
